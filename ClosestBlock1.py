...
def Closest_Block(imgdep, img, getpoint,colorflag):
    ...
    # Colormask = cv2.bitwise_or(cv2.inRange(imghsv, R1[0], R1[1]), cv2.inRange(imghsv, R2[0], R2[1])) if not colorflag else cv2.inRange(imghsv, B[0], B[1])
    '''side only'''
    # Colormask = cv2.inRange(imghsv, R2[0], R2[1]) if not colorflag else cv2.inRange(imghsv, B2[0], B2[1])
    ...
    # 将四周往里面缩一点
    detaUD = int(abs(d - u) / 10)
    detaLR = int(abs(r - l) / 10)
    Rect1 = np.array([[l, u], [l, u+detaUD], [r, u+detaUD], [r, u]])
    Rect2 = np.array([[l, d-detaUD], [l, d], [r, d], [r, d-detaUD]])
    Rect3 = np.array([[l, u], [l, d], [l+detaLR, d], [l+detaLR, u]])
    Rect4 = np.array([[r-detaLR, u], [r-detaLR, d], [r, d], [r, u]])
    ...
    if (cv2.contourArea(cont) * 8 > (r-l) * (d-u)):
        ...
        if imgdep[cY][cX]!=0: return norm, 1,cX,cY,0, mask
        else: return float("inf"),0,cX,cY,0, mask
    elif imgdep[y1][x1]!=0:
        # wid < len
