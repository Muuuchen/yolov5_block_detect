...
from Draw_point import visualization
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    if save_img or view_img:  # Add bbox to image
                        ...
                        cv2.putText(im0, "pixel:{} ".format(time.time()), (100, 50), cv2.FONT_HERSHEY_SIMPLEX, 0.4,
                                    (255, 0, 255))
                        ...
                        for delta in range(20):
                            if mx - delta > 0 and mx + delta < 640:
                                ...
                                theta.append((b - a) / 2)
                                posList.append([mx - delta,my,imdal[i].get_distance(mx - delta, my)])
                                posList.append([mx + delta, my,imdal[i].get_distance(mx + delta, my)])
                        ...
                        for delta in range(20):
                            if my - delta > 0 and my + delta < 480:
                                ...
                                b = (math.acos((x2 ** 2 + l0 ** 2 - l2 ** 2) / (2 * x2 * l0)) / 3.1415926) * 180
                                theta.append((b - a) / 2)
                                posList.append([mx, my - delta, imdal[i].get_distance(mx, my - delta)])
                                posList.append([mx, my + delta, imdal[i].get_distance(mx, my + delta)])
                        theta = np.array(theta[1:])
                        ...
                        print("发送成功")
                        visualization(posList, horres, velres)
